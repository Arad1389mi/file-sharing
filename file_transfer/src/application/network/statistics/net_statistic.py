...
class NetStatistics:
    ...
    def getTransferedFilesByDate(self, date1, date2):
        ...
        conn = sqlite3.connect("D:\\file_transfer\\src\\transfer_logs\\logs.db")
        cursor = conn.cursor()
        
        query = """
        SELECT file_name, transfer_date, size 
        FROM file_transfers 
        ...
        return [{'file_name': row[0], 'transfer_date': row[1], 'size': row[2]} for row in results]
